...
export const Registeruser = async (req, res) => {
  try {
    ...
    const avatarLocalPath = req.files?.avatar?.[0]?.path;
    if (!avatarLocalPath) return res.status(400).json({ success: false, message: "Avatar required" });
    ...
    res.status(201).json({ success: true, message: "User Registered", user: createUser });
  } catch (error) {
    console.log(error);
...
export const LogoutUser = async (req, res) => {
  ...
  res.clearCookie("accessToken", { httpOnly: true, secure: true }).json({ success: true, message: "User logged out" });
};
